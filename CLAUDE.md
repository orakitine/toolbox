...
- **browser-microscope** — Real-browser DOM/layout forensics; the desktop-Playwright counterpart to ios-simulator-microscope. Runs arbitrary JS in a live page, hit-tests why a click won't land, dumps scroll/box geometry, reads live CSS tokens, sweeps viewport widths to find breakpoint bugs. Drives the local Playwright npm package; supports basic-auth and self-signed staging certs. Complements browser/browser-qa.
...
- **ios-simulator-microscope** — Reproduce and measure iOS-Safari-only layout/rendering/interaction bugs on a booted iOS Simulator. Two modes: safaridriver for static measurement, and a session-free reverse-proxy bridge (`sim-bridge.mjs`) for gesture work — CGEvent scrolls raise Safari's automation lock during a WebDriver session, so toolbar-collapse dynamics run through the bridge. The bridge tracks connected pages (`clients`, `--match` eval targeting, `answeredBy` in every answer), auto-wraps eval expressions, captures in-page console/errors, and defeats Safari's sticky cache with `--fresh` origins; `--init-js` installs hooks before page scripts. Paints measurements into screenshots. Runs degraded (no real gestures, JS scrolling still works) without Accessibility permission. The real-device counterpart to browser-microscope.
